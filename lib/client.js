		...
		*/
		function apply(ctx) {
			...
			*  typet remote since the public sessions.create() API does not forward it. */
			const createWslSession = async (workspaceId) => {
				if (sessions.manager?.remote?.session?.create !== void 0) {
			...
			ctx.effect(() => {
				...
				refreshWorkspaces();
				const bindSession = async (id) => {
					const summary = sessions.list.getSnapshot().byId[id];
					if (summary === void 0 || !summary.blank || summary.cwd === void 0) return void 0;
					const canonical = canonicalWindowsPath(summary.cwd);
					if (!(isWslUnc(summary.cwd) || canonical !== null && wslWindowsPaths.has(canonical))) return void 0;
					const current = summary.agentPreset;
					if (current !== void 0 && current.startsWith("wsl-")) return void 0;
					if (hasApi) {
						const base = current === LEGACY_WSL_PRESET_ID ? defaultPreset ?? "standard" : current ?? defaultPreset;
						if (base === void 0 || base === LEGACY_WSL_PRESET_ID || base.startsWith("wsl-")) return void 0;
						const target = `wsl-${base.toLowerCase()}`;
						if (!variants.has(target)) return void 0;
						inFlight.add(id);
						try {
							if ((await api.agentPresets.select({
								sessionId: id,
								agentPreset: target
							})).result.ok) {
					...
					if (sessions.manager?.remote?.session?.create === void 0) return void 0;
					inFlight.add(id);
					try {
				...
				const maybeBind = () => {
					...
					for (const id of state.ids) {
						...
						bindSession(id).then((bound) => {
							if (bound === void 0) attempts.set(id, (attempts.get(id) ?? 0) + 1);
						});
					}
				};
				maybeBind();
